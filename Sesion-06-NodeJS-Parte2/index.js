...
async function cargarConfig() {
    try {
        const texto = await readFile(join(raizProyecto, 'config.env'), 'utf-8');
...
const origen = join(raizProyecto, config.ARCHIVO_ORIGEN || 'data/app.log');
const destino = join(raizProyecto, config.ARCHIVO_DESTINO || 'data/filtrado.log');
